...
{
    public partial class GheymatGozariKala : Form
    {
        public GheymatGozariKala()
        {
            InitializeComponent();
        }
        ...
        private void GheymatGozariKala_Load(object sender, EventArgs e)
        {
            showData();
            bindFields();
        }
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            ...
            if (count != 0)
            {
                ...
                {
                    MessageBox.Show("قیمت فروش وارد نشده");
                    error = true;
                }
                //if (!error)
                //{
                //    long buyPrice = long.Parse(txt_price.Text);
                //    long sellPrice = long.Parse(textBox1.Text);
                //    if (buyPrice >= sellPrice) MessageBox.Show("قیمت فروش کمتر از قیمت خرید است");
                //}
                if (!error)
                {
                    ...
                    sabteKala.add(id,name, type, tarikhSakht, tarikhEngheza, price,tozihat, tedad,tarikh);
                    showData();
                    txt_name.Text = "";
